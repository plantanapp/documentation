...
The new Workflow Builder UI that has been experimental in 1.26 is is now set as the default interface when accessing the Workflow screen. You can still switch to the Legacy UI, which is scheduled for removal in 1.29.

### Introducing Entity List Source for Dropdowns

The Entity List Source feature is available in data source option for dropdown fields within forms, or any other feature that accepts a List Source. This replaces the need to use the SQL data source with an easier, no-code mechanism that doesn't require maintenance since it will take into account changes to the related Entity automatically.
...
### Introduced AI Know-how Assistant

For the Professional Developer plan and above, a chat widget is displayed in the App Builder screen. This offers an interface into an AI assistant trained on over 5,000 articles from our documentation, community forums, support tickets and more.
