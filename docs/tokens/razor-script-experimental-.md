...
### Example 2: Iterate Over and List Portal Properties
...
var portalProperties = portalType.GetProperties().OrderBy(p => p.Name).ToArray();
}
<h2>Portal Properties</h2>
<ul>
@foreach (var prop in portalProperties)
{
var value = "null";
try { var propValue = prop.GetValue(portalInfo, null); value = (propValue != null) ? propValue.ToString() : "null"; }
catch { value = "[error]"; }
<li>@prop.Name: @value</li>
}
</ul>
```
This displays all properties and values of the Portal object.

****

### Example 3: Using @functions for Dynamic Logic
...
### Example 4: Access Token Parameters With Fallback

```csharp
@inherits PlantAnApp.Tokens.Core.RazorTokenBase
@{
object minValue;
try { minValue = TokenParameters.MinimumValue; }
catch { minValue = 0; }
}
<p>Minimum Value Parameter: <b>@minValue</b></p>
```

****

### Example 5: Render a Sub-Page with Data

```csharp
@inherits PlantAnApp.Tokens.Core.RazorTokenBase
<p>Main Page</p>
@RenderPage("SubPage.cshtml", new { ItemId = 123, ItemName = "Example" })
```
Then, in `SubPage.cshtml`:
```csharp
@inherits PlantAnApp.Tokens.Core.RazorTokenBase
<p>Sub Page for item @Model.ItemName (ID: @Model.ItemId)</p>
```

****

### Example 6: Helper to Render Lists
