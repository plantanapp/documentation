...
The action below loads the current user using [Load User](/actions/load-user.md) action and revokes its '[Users]' role. Nothing will happen if a not assigned role is revoked. [Import it](/actions/running-examples.md#import-the-action-in-a-module) into your application to see it in action.
...
The action below loads all users using Load Entities and [Load User](/actions/load-user.md) action and revokes the `Managers` role from every one. [Import it](/actions/running-examples.md#import-the-action-in-a-module) into your application to see it in action.
